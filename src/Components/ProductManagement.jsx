...
const ProductManagement = () => {
  ...
  const [editedProduct, setEditedProduct] = useState(null); // Initialize as null
  ...
  const handleSaveProduct = async () => {
    ...
    try {
      ...
      const response = await axios.patch(
        `http://localhost:8080/products/updateAny?id=${editedProduct.productID}`,
        editedProduct
      );
      setIsEditing(false);
      setEditedProduct(null); // Reset the edited product
      handleGetAllProducts(); // Refresh products
    } catch (error) {
  ...
  const handleDeleteProduct = async (productID) => {
    try {
      // Make the DELETE request without any authorization token
    ...
    <VStack spacing={4} mt={4} w="full" align="stretch">
      ...
      <Flex justify="space-between" wrap="wrap" gap={4}>
        <HStack spacing={4}>
          ...
          <FormControl>
            <FormLabel>Sort by Stock Quantity</FormLabel>
            <Select
            ...
            <Tbody>
              {filteredProducts.map((product) => (
                <Tr key={product.productID}>
                  ...
                  <Td>{product.description}</Td>
                  <Td>{product.sellingPrice}</Td>
                  <Td>{product.stockQuantities[0]}</Td>
                  <Td>
                    <IconButton
                      icon={<HiPencilAlt />}
              ...
              <FormControl mb={4}>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditedProduct({
                      ...editedProduct,
                      stockQuantities: [e.target.value],
                    })
                  }
                />
              </FormControl>
