...
//       stockQuantities: Number(newProduct.stockQuantities), // Convert to number
//     };

//     setProducts([...products, newProductData]);
//     setNewProduct({ productName: '', description: '', price: '', actualPrice: '', sellingPrice: '', discount: '', stockQuantities: '', productType: '' });
...
//           <Input type="number" value={newProduct.stockQuantities} onChange={(e) => setNewProduct({ ...newProduct, stockQuantities: e.target.value })} placeholder="Enter stock quantity" />
...
//         setNewProduct((prev) => ({
//           ...prev,
//           productType: isChecked ? 'READYMADE' : prev.productType === 'READYMADE' ? '' : prev.productType,
//           stockQuantities: isChecked ? prev.stockQuantities : '', // Reset stock quantity when unchecked
//         }));
//       }}
//     >
//       Readymade
...
//       onChange={(e) => setNewProduct({ ...newProduct, stockQuantities: e.target.value })}
...
//                       onChange={(e) => {
//                         const updatedProducts = products.map((p) =>
//                           p.productID === product.productID ? { ...p, stockQuantities: Number(e.target.value) } : p
//                         );
//                         setProducts(updatedProducts);
//                       }}
//                     />
...
//   // Edit a product (fetch product details if needed)
//   const handleEditProduct = (productID) => {
//     setEditableProductID(productID);
//   };

//   // Save edits to the product via API
...
//     try {  console.log(newProduct)
//       const response = await axios.post('http://localhost:8080/products/saveInformation', newProduct);
...
//     <VStack spacing={4} mt={4}>
//       <HStack spacing={4}>
//         <Button colorScheme="teal" onClick={() => setIsFormVisible(true)}>Add Product</Button>
//         <Button colorScheme="blue" onClick={handleGetAllProducts}>Get All Products</Button>
//         <SearchComponent />
//       </HStack>
...
//                 <Checkbox 
//                   isChecked={newProduct.productType === 'READYMADE'} 
//                   onChange={(e) => setNewProduct({ ...newProduct, productType: e.target.checked ? 'READYMADE' : '' })}
//                 >
//                   Readymade
//                 </Checkbox>
//               </FormControl>
...
//                 <Checkbox 
//                   isChecked={newProduct.productType === 'UNSTITCHED'} 
//                   onChange={(e) => setNewProduct({ ...newProduct, productType: e.target.checked ? 'UNSTITCHED' : '' })}
//                 >
//                   Unstitched
//                 </Checkbox>
//               </FormControl>
...
//             <Input value={newProduct.description} onChange={(e) => setNewProduct({ ...newProduct, description: e.target.value })} placeholder="Enter product description" />
//             <FormLabel mt={4}>Price</FormLabel>
//             <Input type="number" value={newProduct.price} onChange={(e) => setNewProduct({ ...newProduct, price: parseFloat(e.target.value) })} placeholder="Enter product price" />
//             <FormLabel mt={4}>Actual Price</FormLabel>
//             <Input type="number" value={newProduct.actualPrice} onChange={(e) => setNewProduct({ ...newProduct, actualPrice: parseFloat(e.target.value) })} placeholder="Enter actual price" />
//             <FormLabel mt={4}>Selling Price</FormLabel>
//             <Input type="number" value={newProduct.sellingPrice} onChange={(e) => setNewProduct({ ...newProduct, sellingPrice: parseFloat(e.target.value) })} placeholder="Enter selling price" />
//             <FormLabel mt={4}>Discount</FormLabel>
//             <Input type="number" value={newProduct.discount} onChange={(e) => setNewProduct({ ...newProduct, discount: parseFloat(e.target.value) })} placeholder="Enter discount" />
//             <FormLabel mt={4}>Stock Quantity</FormLabel>
//             <Input type="number" value={newProduct.stockQuantities} onChange={(e) => setNewProduct({ ...newProduct,   stockQuantities: e.target.value.split(',').map(Number)})} placeholder="Enter stock quantity" />
//           </FormControl>
//           <Button colorScheme="teal" mt={4} onClick={handleAddProduct}>Add Product</Button>
//         </Box>
//       )}
...
const ProductManagement = () => {
  ...
  const handleDeleteProduct = async (productID) => {
    try {
      ...
      await axios.delete(`http://localhost:8080/products/deleteByID?id=${productID}`);
      setProducts(products.filter((product) => product.productID !== productID));
    } catch (error) {
      console.error("Error deleting product:", error);
    }
...
console.log("updatedProduct.........", JSON.stringify(updatedProduct, null, 2));
    ...
    try {
      await axios.patch(`http://localhost:8080/products/updateAny?id=${productID}`, updatedProduct
    ...
    setProducts((prevProducts) =>
      prevProducts.map((product) =>
        product.productID === productID
          ? { ...product, [field]: field === "stockQuantities" ? value.split(',').map(Number) : value }
          : product
      )
    );
    ...
    <VStack spacing={4}>
      <Checkbox
        ...
        onChange={(e) => handleProductChange(
          product.productID, 
          'productType', 
          e.target.checked ? 'READYMADE' : (product.productType === 'UNSTITCHED' ? '' : product.productType)
        )}
      >
        READYMADE
      </Checkbox>
      ...
      <Checkbox
        ...
        onChange={(e) => handleProductChange(
          product.productID, 
          'productType', 
          e.target.checked ? 'UNSTITCHED' : (product.productType === 'READYMADE' ? '' : product.productType)
        )}
      >
        UNSTITCHED
      </Checkbox>
    ...
    product.productType
              ...
              <Td>
                {editableProductID === product.productID ? (
                  <Input
                    type="number"
                    value={product.sellingPrice}
                    onChange={(e) => handleProductChange(product.productID, 'sellingPrice', parseFloat(e.target.value))}
                  />
                ) : (
                  product.sellingPrice
                )}
              ...
              <Td>
                {editableProductID === product.productID ? (
                  <Input
                    value={product.stockQuantities}
                    onChange={(e) => handleProductChange(product.productID, 'stockQuantities', e.target.value)}
